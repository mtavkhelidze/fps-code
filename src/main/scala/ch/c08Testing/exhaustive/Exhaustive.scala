...
object Prop:
  ...
  val p1: Prop = Prop.forAll(Gen.unit(Par.unit(1)))(pi =>
  ...
  val gpy: Gen[Par[Int]] = Gen.choose(0, 10).map(Par.unit)
  val p4: Prop = forAllPar(gpy)(py => equal(py.map(y => y), py))
  val forkProp: Prop = Prop.forAllPar(gpy2)(y => equal(Par.fork(y), y))
  ...
  /* The sized case of `forAll` is as before, though we convert from `Proven` to `Unfalsified`. A sized generator
   * can never be proven, since there are always larger-sized tests that were not run which may have failed.
   */
...
object Gen:
  ...
  /* A `Gen[Par[Int]]` generated from a list summation that spawns a new parallel computation
   * for each element of the input list summed to produce the final result. This is not the most
   * compelling example, but it provides at least some variation in structure to use for testing.
   */
  ...
  val maxProp1: Prop = forAll(smallInt.nonEmptyList): l =>
    val max = l.max
    !l.exists(_ > max) // No value greater than `max` should exist in `l`
  val sortedProp: Prop = Prop.forAll(smallInt.list): l =>
    val ls = l.sorted
  ...
  def genStringFn[A](g: Gen[A]): Gen[String => A] =
    val sample = State[String => A, RNG]: rng =>
      val (seed, rng2) =
